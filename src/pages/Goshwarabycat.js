...
//const instituteId = userInfo?.institute?.id;
  ...
  useEffect(() => {
    fetch(`${config.API_URL}/EmployeeRoster/goshwara_by_cat?s=${userInfo.instituteId}&dt=${selectedDate}`, {
      method: 'GET',
      headers: {
  ...
  }, [userInfo.instituteId,selectedDate]);


  const calculateDistribution = () => {
    let calculated = categories.map((category) => {
      const approvedPosts = Math.round((category.percentage / 100) * totalSeats);
                ...
                <tbody>
                  {distribution.map((cat, index) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-6 border border-gray-400 text-gray-800 font-medium text-right">{cat.approvedPosts}</td>
